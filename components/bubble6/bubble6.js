/**
 * Bubble6 交互：长按默认态 → 展开态浮层
 * 使用 SVG 文件：默认态 151×191，展开态 235×286
 */
(function () {
  var LONG_PRESS_MS = 500;
  var DEFAULT_ID = "bubble6-default";
  var OVERLAY_ID = "bubble6-overlay";
  var BACKDROP_ID = "bubble6-backdrop";
  var EXPANDED_ID = "bubble6-expanded";
  ...
  function showExpanded() {
    ...
    if (document.documentElement.classList.contains("bubble6-embed") && window.parent !== window) {
      try { window.parent.postMessage("bubble6-expanded", "*"); } catch (err) {}
    }
  }
  ...
  function hideExpanded() {
    ...
    if (document.documentElement.classList.contains("bubble6-embed") && window.parent !== window) {
      try { window.parent.postMessage("bubble6-collapsed", "*"); } catch (err) {}
    }
  }
  ...
  if (typeof window !== "undefined" && window.location.search.indexOf("embed=1") !== -1) {
    document.documentElement.classList.add("bubble6-embed");
  }
