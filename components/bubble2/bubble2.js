/**
 * Bubble2 交互：长按默认态 → 展开态浮层
 * 与 bubble2.pen 中「Bubble2-默认态」「Bubble2-展开态」对应
 */
(function () {
  var LONG_PRESS_MS = 500;
  var DEFAULT_ID = "bubble2-default";
  var OVERLAY_ID = "bubble2-overlay";
  var BACKDROP_ID = "bubble2-backdrop";
  var EXPANDED_ID = "bubble2-expanded";
  ...
  function showExpanded() {
    ...
    if (document.documentElement.classList.contains("bubble2-embed") && window.parent !== window) {
      try { window.parent.postMessage("bubble2-expanded", "*"); } catch (err) {}
    }
  }
  ...
  function hideExpanded() {
    ...
    if (document.documentElement.classList.contains("bubble2-embed") && window.parent !== window) {
      try { window.parent.postMessage("bubble2-collapsed", "*"); } catch (err) {}
    }
  }
  ...
  if (typeof window !== "undefined" && window.location.search.indexOf("embed=1") !== -1) {
    document.documentElement.classList.add("bubble2-embed");
  }
